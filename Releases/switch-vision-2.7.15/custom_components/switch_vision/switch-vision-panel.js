...
class SwitchVisionPanel extends HTMLElement {
  ...
  _renderShell() {
    this.shadowRoot.innerHTML = `
      <style>
        :host{display:block;min-height:100%;--sv-panel-header-fg:#eef7ff;--sv-panel-header-bg:#1c242b;background:var(--primary-background-color,#11161b);color:var(--primary-text-color,#e8eef3);font-family:var(--paper-font-body1_-_font-family,Arial,sans-serif)}
        .page{width:100%;max-width:none;box-sizing:border-box;margin:0 auto;padding:10px 16px 40px}
        ...
        .menu-toggle{display:none;align-items:center;justify-content:center;flex:0 0 40px;width:40px;height:40px;padding:0;border-radius:10px;background:var(--sv-panel-header-bg);color:var(--sv-panel-header-fg);font-size:27px;font-weight:400;line-height:1}
        .menu-toggle:hover,.menu-toggle:focus-visible{background:var(--secondary-background-color,rgba(127,127,127,.18));outline:none}
        .summary{flex:1;min-width:0;border-left:4px solid #3e8fc5;border-radius:8px;padding:9px 12px;background:var(--sv-panel-header-bg);color:var(--sv-panel-header-fg);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .version{font-size:12px;border:1px solid rgba(127,180,220,.45);border-radius:999px;padding:5px 9px;white-space:nowrap;background:var(--sv-panel-header-bg);color:var(--sv-panel-header-fg)}
