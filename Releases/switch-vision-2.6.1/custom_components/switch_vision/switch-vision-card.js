...
function geometryTransferPick(source, keys) {
  ...
  for (const key of keys) {
    if (source[key] !== undefined) out[key] = cloneCalibrationData(source[key]);
  }
  return out;
}
...
function geometryTransferBox(source, { fields = false, anchor = false } = {}) {
  ...
  if (fields && source?.fields && typeof source.fields === "object" && !Array.isArray(source.fields)) {
    out.fields = cloneCalibrationData(source.fields);
  }
...
function geometryTransferExportData(cal, { scope, baseProfile, profile } = {}) {
  ...
  for (const [key, point] of Object.entries(source.status_leds || {})) {
    ...
    statusLeds[key] = cloneCalibrationData(point);
  }
  return {
    schema_version: 1,
...
function applyGeometryTransferData(currentCal, raw) {
  ...
  for (const key of ["width", "height"]) {
    if (imageGeometry[key] !== undefined) next.image[key] = cloneCalibrationData(imageGeometry[key]);
  }
  ...
  for (const key of geometryTransferKeySet(current.status_leds, { ignoreMode: true })) {
    next.status_leds[key] = cloneCalibrationData(geometry.status_leds[key]);
  }
  ...
  applied.ui.faceplate = cloneCalibrationData(current.ui?.faceplate || {});
