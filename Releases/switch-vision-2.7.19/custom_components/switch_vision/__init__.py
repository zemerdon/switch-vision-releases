...
UI_DENSITY_SCHEMA = vol.Schema(
    {
        vol.Required("density"): vol.In(list(UI_DENSITY_VALUES)),
    }
)
...
DELETE_WS_SCHEMA = {
    vol.Required("type"): "switch_vision/delete_calibration",
    vol.Required("profile"): vol.All(str, vol.Length(min=1)),
}
...
GET_BACKUP_ASSET_WS_SCHEMA = {
    vol.Required("type"): "switch_vision/get_backup_asset",
    vol.Required("kind"): vol.All(str, vol.Length(min=1, max=32)),
    vol.Required("filename"): vol.All(str, vol.Length(min=1, max=255)),
}

PUT_BACKUP_ASSET_WS_SCHEMA = {
    vol.Required("type"): "switch_vision/put_backup_asset",
    vol.Required("kind"): vol.All(str, vol.Length(min=1, max=32)),
    vol.Required("filename"): vol.All(str, vol.Length(min=1, max=255)),
    vol.Required("sha256"): vol.Match(r"^[0-9a-f]{64}$"),
...
FACEPLATE_WIDTH_MODES = ("auto", "800", "1024", "custom")
FACEPLATE_WIDTH_MIN_PX = 320
FACEPLATE_WIDTH_MAX_PX = 4096
FACEPLATE_WIDTH_DEFAULT_PX = 800
...
DEFAULT_OPTIONS = {
    ...
    CONF_FACEPLATE_WIDTH_MODE: "auto",
...
STOCK_ASSET_MANIFEST_PATH = Path(__file__).with_name("stock-assets.json")


def _load_stock_asset_manifest() -> dict[str, dict[str, str]]:
    """Return the packaged stock visual-asset SHA-256 manifest."""
    payload = json.loads(STOCK_ASSET_MANIFEST_PATH.read_text(encoding="utf-8"))
    if payload.get("schema") != "switch-vision-stock-assets-v1":
        raise ValueError("Switch Vision stock asset manifest schema is invalid.")
    assets = payload.get("assets")
    if not isinstance(assets, dict):
        raise ValueError("Switch Vision stock asset manifest is invalid.")
    result: dict[str, dict[str, str]] = {}
    for kind in ASSET_KINDS:
        rows = assets.get(kind)
        if not isinstance(rows, dict):
            raise ValueError(f"Switch Vision stock asset manifest is missing {kind}.")
        clean: dict[str, str] = {}
        for raw_name, raw_digest in rows.items():
            ...
            if (
                ...
                or name.startswith(".")
                or Path(name).suffix.lower() not in ASSET_EXTENSIONS
                or len(digest) != 64
                or any(ch not in "0123456789abcdef" for ch in digest)
            ):
                raise ValueError("Switch Vision stock asset manifest contains an invalid entry.")
            clean[name] = digest
        result[kind] = clean
    return result
...
MAX_BACKUP_ASSET_BYTES = 16 * 1024 * 1024
...
CORE_SETTINGS_ENUMS = {
    CONF_FACEPLATE_WIDTH_MODE: frozenset(FACEPLATE_WIDTH_MODES),
...
CORE_SETTINGS_INT_RANGES = {
    CONF_FACEPLATE_CUSTOM_WIDTH: (FACEPLATE_WIDTH_MIN_PX, FACEPLATE_WIDTH_MAX_PX),
...
def _faceplate_width_settings(entry: ConfigEntry) -> dict[str, Any]:
    mode = str(entry.options.get(CONF_FACEPLATE_WIDTH_MODE, "auto"))
    if mode not in FACEPLATE_WIDTH_MODES:
        mode = "auto"
    raw_custom = entry.options.get(CONF_FACEPLATE_CUSTOM_WIDTH, FACEPLATE_WIDTH_DEFAULT_PX)
    custom = raw_custom if isinstance(raw_custom, int) and not isinstance(raw_custom, bool) else FACEPLATE_WIDTH_DEFAULT_PX
    custom = max(FACEPLATE_WIDTH_MIN_PX, min(FACEPLATE_WIDTH_MAX_PX, custom))
    effective = None if mode == "auto" else (custom if mode == "custom" else int(mode))
...
LEGACY_UNIFI_32SFP_PROFILE = "unifi_28sfp"
CANONICAL_UNIFI_32SFP_PROFILE = "unifi_32sfp"
LEGACY_UNIFI_32SFP_FACEPLATE = "unifi-28sfp.png"
CANONICAL_UNIFI_32SFP_FACEPLATE = "unifi-32sfp.png"
LEGACY_UNIFI_32SFP_TOKEN = "unifi-28sfp-png-de858a65"
CANONICAL_UNIFI_32SFP_TOKEN = "unifi-32sfp-png-857125f4"


def _canonical_unifi_32sfp_profile_name(value: Any) -> str:
    name = str(value or "")
    if name == LEGACY_UNIFI_32SFP_PROFILE:
        return CANONICAL_UNIFI_32SFP_PROFILE
    suffix = f"__faceplate__{LEGACY_UNIFI_32SFP_TOKEN}"
    if name.endswith(suffix):
        return name[: -len(suffix)] + f"__faceplate__{CANONICAL_UNIFI_32SFP_TOKEN}"
    return name


def _canonical_unifi_32sfp_faceplate_ref(value: Any) -> str:
    text = str(value or "")
    if text == LEGACY_UNIFI_32SFP_FACEPLATE:
        return CANONICAL_UNIFI_32SFP_FACEPLATE
    if text == f"faceplates/{LEGACY_UNIFI_32SFP_FACEPLATE}":
        return f"faceplates/{CANONICAL_UNIFI_32SFP_FACEPLATE}"
    return text


def _canonicalise_unifi_32sfp_calibration(calibration: Any) -> bool:
    if not isinstance(calibration, dict):
        return False
    changed = False
    for key in ("profile", "profile_name", "base_profile_name", "source_profile", "source_base_profile"):
        if key not in calibration:
            continue
        current = str(calibration.get(key) or "")
        canonical = _canonical_unifi_32sfp_profile_name(current)
        if canonical != current:
            calibration[key] = canonical
            changed = True
    if calibration.get("model") == "unifi-28sfp":
        calibration["model"] = "unifi-32sfp"
        changed = True
    for key in ("faceplate_file", "required_faceplate"):
        if key not in calibration:
            continue
        current = str(calibration.get(key) or "")
        canonical = _canonical_unifi_32sfp_faceplate_ref(current)
        if canonical != current:
            calibration[key] = canonical
            changed = True
    image = calibration.get("image")
    if isinstance(image, dict):
        current = str(image.get("file") or "")
        canonical = _canonical_unifi_32sfp_faceplate_ref(current)
        if canonical != current:
            image["file"] = canonical
            changed = True
        if image.get("master") == "unifi-28sfp-owner-calibrated":
            image["master"] = "unifi-32sfp-owner-calibrated"
            changed = True
    ui = calibration.get("ui")
    faceplate = ui.get("faceplate") if isinstance(ui, dict) else None
    if isinstance(faceplate, dict):
        current = str(faceplate.get("file") or "")
        canonical = _canonical_unifi_32sfp_faceplate_ref(current)
        if canonical != current:
            faceplate["file"] = canonical
            changed = True
    return changed


def _migrate_unifi_32sfp_identity(data: dict[str, Any]) -> bool:
    # Seed canonical 32-port calibration keys from legacy 28-name state.
    profiles = data.setdefault("profiles", {})
    active_profiles = data.setdefault("active_profiles", {})
    if not isinstance(profiles, dict) or not isinstance(active_profiles, dict):
        return False
    changed = False
    for legacy_name in list(profiles):
        canonical_name = _canonical_unifi_32sfp_profile_name(legacy_name)
        if canonical_name == legacy_name:
            continue
        legacy_value = profiles.get(legacy_name)
        if canonical_name not in profiles:
            canonical_value = json.loads(json.dumps(legacy_value))
            _canonicalise_unifi_32sfp_calibration(canonical_value)
            profiles[canonical_name] = canonical_value
            changed = True
    for profile_name, calibration in list(profiles.items()):
        if _canonical_unifi_32sfp_profile_name(profile_name) == profile_name:
            if _canonicalise_unifi_32sfp_calibration(calibration):
                changed = True
    for base_name, active_name in list(active_profiles.items()):
        canonical_base = _canonical_unifi_32sfp_profile_name(base_name)
        canonical_active = _canonical_unifi_32sfp_profile_name(active_name)
        if canonical_base != base_name and canonical_base not in active_profiles:
            active_profiles[canonical_base] = canonical_active
            changed = True
        if canonical_active != str(active_name or ""):
            active_profiles[base_name] = canonical_active
            changed = True
    return changed
...
def _read_unifi_device_snapshot(path: Path, device_id: str) -> dict[str, Any]:
    ...
    for raw in document["devices"]:
        ...
        return {
            ...
            "stale_after_seconds": document.get("stale_after_seconds"),
        }
...
async def async_setup(hass: HomeAssistant, config: dict[str, Any]) -> bool:
    ...
    async def _load() -> dict[str, Any]:
        ...
        migrated_mirrors = _migrate_legacy_faceplate_mirrors(data)
        migrated_unifi_32sfp = _migrate_unifi_32sfp_identity(data)
        if migrated_mirrors or migrated_unifi_32sfp:
            await store.async_save(data)
            if migrated_mirrors:
                _LOGGER.info("Migrated legacy Switch Vision faceplate calibration mirrors to isolated active-profile pointers")
            if migrated_unifi_32sfp:
                _LOGGER.info("Migrated legacy UniFi 28SFP calibration identity to canonical UniFi 32-port optical identity")
        return data
    ...
    def _list_assets() -> dict[str, Any]:
        ...
        warnings: list[str] = []
        try:
            stock_assets = _load_stock_asset_manifest()
        except (OSError, ValueError, json.JSONDecodeError) as err:
            stock_assets = None
            warnings.append(f"Could not load the Switch Vision stock asset manifest: {err}")
        for kind in ASSET_KINDS:
            folder = root / kind
            files: list[str] = []
            ...
            try:
                ...
                if stock_assets is not None:
                    stock_kind = stock_assets[kind]
                    for path in paths:
                        expected = stock_kind.get(path.name)
                        if expected is None:
                            custom_files.append(path.name)
                            continue
                        try:
                            digest = hashlib.sha256(path.read_bytes()).hexdigest()
                        except OSError as err:
                            warnings.append(f"Could not fingerprint {path}: {err}")
                            custom_files.append(path.name)
                            continue
                        if digest != expected:
                            # A user-modified stock-named file is still user data and
                            # must travel in a complete portable backup.
                            custom_files.append(path.name)
            except OSError as err:
            ...
            result[kind] = files
            result[f"custom_{kind}"] = custom_files
            result[f"{kind}_path"] = f"/config/www/switch-vision/{kind}"
        result["extensions"] = sorted(ASSET_EXTENSIONS)
        result["backup_api"] = 2 if stock_assets is not None else 1
        result["warnings"] = warnings
        return result

    def _backup_asset_path(kind: Any, filename: Any) -> Path:
        selected_kind = str(kind or "").strip().lower()
        if selected_kind not in ASSET_KINDS:
            raise ValueError("Unsupported Switch Vision asset kind.")
        selected_name = str(filename or "").strip()
        if (
            not selected_name
            or len(selected_name) > MAX_ASSET_FILENAME_LENGTH
            or selected_name != Path(selected_name).name
            or selected_name.startswith(".")
            or Path(selected_name).suffix.lower() not in ASSET_EXTENSIONS
        ):
            raise ValueError("Switch Vision asset filename is invalid.")
        return _asset_root() / selected_kind / selected_name

    def _read_backup_asset(kind: Any, filename: Any) -> tuple[bytes, str]:
        path = _backup_asset_path(kind, filename)
        if not path.is_file():
            raise FileNotFoundError(path.name)
        data = path.read_bytes()
        if len(data) > MAX_BACKUP_ASSET_BYTES:
            raise ValueError("Switch Vision asset exceeds the backup size limit.")
        return data, hashlib.sha256(data).hexdigest()

    def _write_backup_asset(kind: Any, filename: Any, encoded: Any, expected_sha256: Any) -> dict[str, Any]:
        path = _backup_asset_path(kind, filename)
        text = str(encoded or "")
        try:
            data = base64.b64decode(text.encode("ascii"), validate=True)
        except (UnicodeEncodeError, binascii.Error, ValueError) as err:
            raise ValueError("Switch Vision backup asset content is not valid base64.") from err
        if len(data) > MAX_BACKUP_ASSET_BYTES:
            raise ValueError("Switch Vision backup asset exceeds the restore size limit.")
        digest = hashlib.sha256(data).hexdigest()
        if digest != str(expected_sha256 or "").strip().lower():
            raise ValueError("Switch Vision backup asset SHA-256 does not match the manifest.")
        ...
        try:
            ...
            try:
                ...
                pass
        return {"kind": str(kind), "filename": path.name, "size": len(data), "sha256": digest}
    ...
    async def _delete_calibration_profile(profile_value: str) -> dict[str, Any]:
        profile = _validate_profile_name(profile_value)
        base_profile = ""
    ...
    @websocket_api.websocket_command(DELETE_WS_SCHEMA)
    @websocket_api.require_admin
    @websocket_api.async_response
    async def websocket_delete_calibration(hass: HomeAssistant, connection, msg):
        """Delete one unused Switch Vision calibration profile."""
        try:
            result = await _delete_calibration_profile(msg["profile"])
    ...
    @websocket_api.websocket_command(GET_BACKUP_ASSET_WS_SCHEMA)
    @websocket_api.require_admin
    @websocket_api.async_response
    async def websocket_get_backup_asset(hass: HomeAssistant, connection, msg):
        """Return one bounded Switch Vision visual asset for a local backup."""
        try:
            data, digest = await hass.async_add_executor_job(
                _read_backup_asset, msg["kind"], msg["filename"]
            )
        except FileNotFoundError:
            connection.send_error(msg["id"], "not_found", "Switch Vision asset was not found")
            return
    ...
    @websocket_api.websocket_command(PUT_BACKUP_ASSET_WS_SCHEMA)
    @websocket_api.require_admin
    @websocket_api.async_response
    async def websocket_put_backup_asset(hass: HomeAssistant, connection, msg):
        """Restore one validated Switch Vision visual asset from a local backup."""
        try:
    ...
    async def websocket_get_ui_settings(hass: HomeAssistant, connection, msg):
        ...
        connection.send_result(
            msg["id"],
            {
                ...
                "faceplate_width": dict(domain_state.get(DATA_FACEPLATE_WIDTH_SETTINGS, {"mode": "800", "custom": 800, "effective": 800})),
                "activity_leds": dict(
                    domain_state.get(
    ...
    websocket_api.async_register_command(hass, websocket_get_backup_asset)
    websocket_api.async_register_command(hass, websocket_put_backup_asset)
