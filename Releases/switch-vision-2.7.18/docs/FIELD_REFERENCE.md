...
Faceplate height is a Calibration-only presentation setting stored as `ui.faceplate.max_height` in the saved switch/card profile. **Auto** is the default and leaves the field unset, preserving the existing rendering. Calibration offers **Compact (115 px)**, **Medium (150 px)**, **Large (200 px)**, and **Custom**; Custom is clamped to a hard minimum of **80 px** and maximum of **1024 px**. A selected height changes only the rendered faceplate height; the card keeps its normal width, and the image/SVG presentation is vertically mapped to the chosen height so ports and labels stay aligned. This is not a Lovelace/YAML card field.
