...
function drawPanel(svg, { hass, config, calibration, layout }) {
  ...
  for (const [num, port] of Object.entries(calibration.ports)) {
    ...
    if (config.show_port_leds) {
      portLed(svg, port.led_left[0], port.led_left[1], layout.ports.r, speedClass(speed, up), calibration, port.led_left_size, "led_left");

      portLed(svg, port.led_right[0], port.led_right[1], layout.ports.r, activityClass(config, up && testPortActivity(hass, config, n)), calibration, port.led_right_size, "led_right");
    }
  ...
  for (const [name, sfp] of Object.entries(calibration.sfp)) {
    ...
    sfpLed(svg, sfp, "led_left", speedClass(String(liveSfpSpeed || 10000), up), calibration);
    sfpLed(svg, sfp, "led_right", activityClass(config, up && testSfpActivity(hass, config, sfpPort)), calibration);
  }
...
<div class="cv-cal-tools-row cv-cal-switch-stack-row">
  ...
  render() {
    ...
    const faceplateUrl = assetUrl("faceplates", faceplateFilename || this.recommendedFaceplateFilename(activeCalibration));
    const recommendedFaceplateUrl = assetUrl("faceplates", this.recommendedFaceplateFilename(activeCalibration));
    ...
    for (const button of this.shadowRoot.querySelectorAll('[data-cv-action]')) {
      button.addEventListener("click", async (event) => {
        ...
        if (action === "cancel-calibration") {
          ...
          this._calibrationDirty = false;
          this.config = { ...this.config, calibration_mode: false, calibration_controls: false };
