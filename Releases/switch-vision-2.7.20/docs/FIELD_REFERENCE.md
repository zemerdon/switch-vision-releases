...
| `activity_slow_period_ms` | Average cadence for Slow activity flicker. Larger values make light-traffic flashes less frequent. Factory default is `500`. |
| `activity_medium_period_ms` | Average cadence for Medium activity flicker. Larger values spread flashes farther apart. Factory default is `250`. |
| `activity_fast_period_ms` | Average cadence for Fast activity flicker. Heavy traffic remains mostly lit with short irregular drop-outs. Factory default is `120`. |
| `activity_hold_seconds` | Keeps the last real activity sample valid between traffic samples; it does not generate synthetic traffic. Factory default is `12` seconds for the current 10-second traffic cadence. |
...
Card height is a Calibration-only presentation setting stored as `ui.faceplate.max_height` in the saved switch/card profile. **Auto** is the default and leaves the field unset, preserving the existing rendering. Calibration offers **Compact (115 px)**, **Medium (150 px)**, **Large (200 px)**, and **Custom**; Custom is clamped to a hard minimum of **80 px** and maximum of **1024 px**. A fixed value changes the card viewport height only: the faceplate image keeps its normal width and aspect ratio, the SVG overlay remains on the normal 2048×448 mapping, and the viewport centrally crops any overflow above/below the selected height. This is not a Lovelace/YAML card field.
...
Calibration font-size inputs always use effective **2048×448 render-space pixels**. Native-image profiles may store larger internal scalar values so their font geometry remains proportional to the source artwork (for example, a 6000×1325 profile stores 13 rendered pixels as 38.0859375 native units), but Calibration hides that conversion. Editing 13 means 13 rendered pixels, and Switch Vision converts to/from native storage automatically for RJ45 numbers, SFP/uplink labels, Status LED labels, and both Status Boxes.
